...
def scaleCntnueVariable(tableName , values):
    for value in values:
        #tableName[value] = (tableName[value] - tableName[value].min()) / (tableName[value].max() - tableName[value].min())
...
def relationToNetwork(objectRelationship,numOfUser,ifHasitsOwn,ifBIGDATA,prefix , fileplace):
    ...
    if ifBIGDATA == False:
        ...
        objectNetwork = csr_matrix((objectRelationship.loc[:,columns[2]],
                                    ...
                                    ) ,shape = (numOfUser,numOfUser),dtype=np.float32)
    ...
    with  h5sparse.File(netFilePlace + prefix+"dot_cosine.h5") as h5f:
        ...
        for i,j in enumerate(sep):
            if i + 1 < len(sep):
                ...
                # this will set the sparse attr in the dis matrix
                ...
                # this may not be used in small data set
                if (len(HasObjectWithNoCntnue) != 0):
                    dis[HasObjectWithNoCntnue - j, :] = -1e6
